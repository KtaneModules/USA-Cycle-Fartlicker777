...
public class USACycle : MonoBehaviour {
   ...
   string[] LetterAbbreviations = "AL AK AZ AR CA CO CT DE FL GA HI ID IL IN IA KS KY LA ME MD MA MI MN MS MO MT NB NV NH NJ NM NY NC ND OH OK OR PA RI SC SD TN TX UT VT VA WA WV WI WY".Split(' ');
   ...
   private KeyCode[] TheKeys =
  {
         ...
         if (!InputMode) {
            ...
            if (CheckIfWordIsValid()) {
               ...
               if (Rnd.Range(0, 20) == 0) {
                  ...
                  Audio.PlaySoundAtTransform("Anthem", transform);
               }
            }
            else {
   ...
   float ResizeStates (string Input) {
      if (Input == "Wisconsin") {
         return .0015f; //What the fuck Wisconsin
      }
